...
in timestamp order and reports the first row at which the chain breaks.

HARD INVARIANTS (both equally critical):
  ...
  2. Every column referenced by _payload_for() must be set BEFORE
     ...
     explicitly (str(uuid.uuid4())) at construction time; relying on the
     SQLAlchemy column default would leave it None during hashing, because
     Python-side defaults fire at flush time, not at object construction.
...
def record_audit(
    ...
    *,
    ...
    prev = (
    db.query(AuditLog)
    .order_by(desc(AuditLog.timestamp), desc(AuditLog.audit_id))
    .first()
    )
...
def verify_audit_chain(db: Session) -> dict:
    """Walk the audit chain in timestamp order and verify integrity.

    Ordering: (timestamp ASC, audit_id ASC). Timestamp is the primary
    ordering key; audit_id is the deterministic tie-breaker.

    Failure classifications (first broken row wins):
      - chain_break_timestamp_inversion
      - chain_break_insertion_or_deletion
      - hash_mismatch
    """
    rows = (
        db.query(AuditLog)
        .order_by(asc(AuditLog.timestamp), asc(AuditLog.audit_id))
        .all()
    )
